...
class Drone:
    ...
    def dist(self, target):
        d = (target[0]-self.gps.getValues()[0])**2 + (target[2]-self.gps.getValues()[2])**2
        return d    
    def vector(self, target):
        # v = (target[0]-self.gps.getValues()[0])/(target[2]-self.gps.getValues()[2])
        angle = math.atan2(target[0]-self.gps.getValues()[0],target[2]-self.gps.getValues()[2])*180/3.14 
        # if(angle < 0):
            # angle = angle + 360
        return angle   
...
i=0
c=0
target = [38.9 , -0.0087 , -229]
while drone.robot.step(drone.timestep) != -1:
    target = [93.9 , 8.29 , -170]
