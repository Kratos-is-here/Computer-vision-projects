...
class Drone:
    ...
    def ocr(self, frame):
        ...
        green_mask = cv2.erode(green_mask, (2, 2), iterations=3)
        ...
        if (yellow_center1[0] < green_center1[0]):
            ...
            # print('w=',w)
            crop_img = frame[y - w:y, x:x + w]
            Matrix = cv2.getRotationMatrix2D(green_center1, 180, 1.0)
            
            #crop_img = cv2.warpAffine(crop_img, Matrix, (500, 500))
            ...
            crop_img = cv2.warpAffine(crop_img, Matrix, (500, 500))
            cv2.imshow('letter', crop_img)
            ret, crop_img = cv2.threshold(crop_img, 200, 255, cv2.THRESH_BINARY)
            edged = cv2.Canny(crop_img, 30, 200)
            # cv2.imshow('crop_img', crop_img)
            # dc, contours,abc = cv2.findContours(edged,cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
            # c = max(contours, key=cv2.contourArea)
    ...
    def goto(self, target, i,sexc):
        ...
        if (d <= 1):
            ...
            if (self.qrscan(self.get_image())):
                ...
                print('hwuwhuhuw')
                if(target[0]!=38.85):
                    cv2.imwrite('i2.png',self.get_image())
                    i2=cv2.imread('i2.png')
                    cv2.imshow('i2',i2)
                    cv2.waitKey(0)
                    print("srikar is boss ")
...
while drone.robot.step(drone.timestep) != -1:
    ...
    drone.goto(drone.target, i,sexc)
